...
> 이 문서는 특정 시점의 스냅샷이다. `main`이 이후 갱신되면 아래 내용을 다시 확인한다.
...
3. 이후 API 요청에 `Authorization: ****** 사용한다.
...
인증/식별 계약의 전체 구조는 `docs/architecture.md` 3절을 함께 참고한다.
...
기능별 Mock/실제 연동 범위의 더 자세한 표는 `docs/product-overview.md`의
"구현 / 미연결 범위" 절도 함께 참고한다.
...
- Vercel Production은 `main`으로 배포되어 `Ready / Current` 상태다.
