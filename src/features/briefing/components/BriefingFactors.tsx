...
export function BriefingFactors({ metrics }: BriefingFactorsProps) {
  return (
    <section>
      <div className="mb-3">
        <h2 className="text-[17px] font-bold tracking-[-0.025em] text-ez-text">오늘 이런 점을 함께 봤어요</h2>
      </div>
        ...
        {metrics.map((metric) => {
          ...
          return (
            <div key={metric.id} className="flex min-h-[72px] items-center gap-3 py-3">
              <span className="grid size-9 shrink-0 place-items-center rounded-[13px] bg-ez-primary-soft text-ez-primary">
