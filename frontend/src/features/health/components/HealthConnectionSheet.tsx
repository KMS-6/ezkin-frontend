...
export function HealthConnectionSheet({
  ...
  const connectedMetrics = getAvailableHealthMetricLabels(connection.availableMetrics)
  const visibleMetrics = isConnected ? connectedMetrics : ['수면', 'HRV']
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#191525]/25 px-0 sm:items-center sm:p-4" role="presentation" onMouseDown={() => !isBusy && onClose()}>
      ...
      >
        <div className="flex items-center justify-between">
          <p className="text-[11px] font-semibold text-ez-primary">워치 연결</p>
        ...
        {isConfirmingDisconnect ? (
          <div className="pt-3">
            <h2 id="health-connection-title" className="text-[19px] font-bold tracking-[-0.025em] text-ez-text">워치 연결을 끊을까요?</h2>
            <p className="mt-2 text-[13px] leading-5 text-ez-muted">기기의 건강 데이터는 지워지지 않고 EZkin과의 연결만 끊어요.</p>
          ...
          <div className="pt-3">
            ...
            <h2 id="health-connection-title" className="mt-4 text-[19px] font-bold tracking-[-0.025em] text-ez-text">
              {isConnected ? '워치가 연결됐어요.' : '워치를 연결할까요?'}
            </h2>
            <p className="mt-2 text-[13px] leading-5 text-ez-muted">
              {isConnected
                ? connectedMetrics.length > 0
                  ? `${connectedMetrics.join(' · ')}를 함께 보고 있어요.`
                  : '연결됐지만 가져온 데이터는 아직 없어요.'
                : '수면과 HRV 흐름을 함께 볼 수 있어요.'}
            </p>

            {visibleMetrics.length > 0 && <div className="mt-5 grid grid-cols-2 gap-2" aria-label="사용할 생활 데이터">
